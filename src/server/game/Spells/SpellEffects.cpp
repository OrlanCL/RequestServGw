    ...
    switch (m_spellInfo->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            switch (m_spellInfo->Id)
            {
				...
				// Shadowmeld - added by Orlan
				case 58984:
				{
					m_caster->InterruptSpell(CURRENT_AUTOREPEAT_SPELL); // Interrumpe autodisparo
					m_caster->InterruptSpell(CURRENT_CHANNELED_SPELL);  // Interrumpe canalizados
					m_caster->AttackStop();
